...
Methodology: Re-enumerated the macOS 26.2 Network.framework C public surface from 32 headers, including nw_* and sec_* functions, opaque types, and constants. Cross-referenced against the crate's Swift bridge (320 @_cdecl interop functions) and Rust safe API to identify verified symbols. No macOS-unavailable API_UNAVAILABLE(macos) symbols were included. Both v1 gaps have now been closed in v2: nw_error_copy_cf_error is now safely exposed via FrameworkError::copy_cf_error() returning apple_cf::CFError, and nw_ws_metadata_set_pong_handler is now exposed via WebSocket::set_pong_handler() and WebSocket::set_pong_handler_with_context().
...
| `nw_error_copy_cf_error` | function | `error.h` | FrameworkError::copy_cf_error() → apple_cf::CFError |
...
| `nw_ws_metadata_set_pong_handler` | function | `ws_options.h` | WebSocket::set_pong_handler() / set_pong_handler_with_context() |
...
- **Gap 1 (nw_error_copy_cf_error)**: Now implemented. Added `FrameworkError::copy_cf_error()` which safely wraps the underlying C function and returns a `apple_cf::cf::CFError` owned handle using the `from_raw_retained` constructor.
- **Gap 2 (nw_ws_metadata_set_pong_handler)**: Now implemented. Added `WebSocket::set_pong_handler()` and `WebSocket::set_pong_handler_with_context()` which provide safe wrappers for registering pong callbacks on WebSocket metadata.
- **No new gaps introduced**: All 498 previously verified symbols remain verified and 2 additional symbols now verified.
- **No exemptions apply**: All 500 symbols are confirmed public, available on current macOS, and now fully covered.
- **Version bump**: Minor bump (0.9.3 → 0.10.0) for new public APIs (copy_cf_error, set_pong_handler).
- **Dependency added**: apple-cf v0.7.0 for safe CFError wrapper type.
